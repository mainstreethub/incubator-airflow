...
def backfill_past(dag_id, dates=None):
    ...
    if dag.parent_dag is None:
        dagruns = backfill_dagruns(session, dag_id, start_date, dates)
    ...
    stats = {dag_id: {"created": len(tis_to_create), "updated": len(tis_to_change), "dagruns": len(dagruns)}}
...
def backfill_dagruns(session, dag_id, start_date, dates):
    DR = DagRun
    existing_dagruns = session.query(DR).filter(
        DR.dag_id == dag_id,
        DR.execution_date.in_(dates)).all()
    existing_dates = set([dr.execution_date for dr in existing_dagruns])
    dates_to_create = [d for d in dates if d not in existing_dates]

    for ed in dates_to_create:
        dr = DagRun(
            dag_id=dag_id,
            execution_date=ed,
            state=State.SUCCESS,
            external_trigger=False,
            run_id='scheduled__' + ed.isoformat(),
